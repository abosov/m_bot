...
## 8) Приватный контур редактирования профиля (draft)
Для редактирования профиля специалиста используется отдельный приватный web-контур, не связанный с Public Specialist API:
- entrypoint из личного Telegram-бота: `/profile/edit#token=<web_connect_token>`;
- frontend потребляет token через `POST /auth/telegram/consume`;
- backend устанавливает `web_auth_session` cookie;
