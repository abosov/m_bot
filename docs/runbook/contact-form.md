...
## 5) Автоответ пользователю

После успешной отправки contact-form backend отправляет пользователю письмо-подтверждение на email из формы.

Условия:
- автоответ отправляется **только** если основное письмо (`CONTACT_TO_EMAIL`/`CONTACT_SMTP_TO`) отправилось успешно;
- отправка автоответа управляется env `CONTACT_AUTOREPLY_ENABLED`:
  - `true` (по умолчанию) — автоответ включён;
  - `false` — автоответ не отправляется;
- если автоответ не отправился, основной ответ endpoint не ломается и остаётся `{"ok": true}`.

Технически:
- используется **тот же SMTP**-клиент и те же SMTP env-параметры;
- `CONTACT_SMTP_PORT` для VPS рекомендуется `2525`;
- timeout отправки (и основного письма, и автоответа) берётся из `CONTACT_SMTP_TIMEOUT_SECONDS`.
...
```text
Мы получили ваше сообщение — Zumbot
```
...
-----------------------
Хочу подключить бота
-----------------------

Номер обращения: req_123456789
