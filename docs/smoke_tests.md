...
### Шаг 3: mobile header не выезжает за экран
Открыть главную страницу на ширине `<= 900px` и проверить:
- в шапке отображается короткий бренд `Zumbot`;
- навигация переносится на следующую строку;
- нет горизонтального скролла/обрезания пунктов меню.
...
- после успешного выбора календаря и интеграции;
- при явной проверке интеграции;
