...
Доп. проверка слотов: убедиться, что применяется `slot_step_min` (только `{60,30,15,10}`, дефолт 15), буфер используется только в вычислении, стык-в-стык интервалы склеиваются, и действует `max_sessions_per_day` (дефолт 4).
