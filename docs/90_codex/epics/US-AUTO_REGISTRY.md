...
1. US-AUTO-75 — additive review-fidelity projection contract for semantic companion filtering
...
| US-AUTO-75 | Additive review-fidelity projection contract for semantic companion filtering | Introduce producer-owned `semantic_projection.json` and integrate it as a preferred downstream validation fast-path without rewriting recompute/manual-finish/stale-surface fallback contracts | follow-up | In Progress | P1 | Converge implementation and reach committed-head review-chain completion on current branch | Follow-up from blocked US-AUTO-74 line | automation/bundle_packs/US-AUTO-75.bundle.md | Active contract-level continuation. Critical constraint: additive integration only; no projection-only rewrite; preserve producer scope/rollback/signal-handling contracts and downstream recompute/manual-finish/stale-surface fallback behavior. |
