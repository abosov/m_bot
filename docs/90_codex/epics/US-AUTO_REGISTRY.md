...
- A new contract-level gap was confirmed while attempting US-AUTO-73: semantic companion filtering does not converge under committed-HEAD rerun when non-runtime companion classification is duplicated across producer and downstream review consumers; this requires centralization into one shared source of truth before US-AUTO-73 can resume.
...
- Attempted implementation of US-AUTO-73 confirmed a contract-level non-converging pattern rather than a local scope bug: semantic non-runtime companion filtering drifted across producer and downstream consumers on committed-HEAD rerun because the classification logic was duplicated instead of centralized.
...
1. US-AUTO-74 — centralize semantic companion-filter contract
...
| US-AUTO-73 | Refine companion-filter semantics for mixed-scope stories | Replace crude "code-only vs mixed-scope" heuristic with path-semantics-based classification of companion artifacts so supporting docs do not invalidate companion-filter validation | governance | Blocked | P1 | Resume only after US-AUTO-74 lands | Follow-up from US-AUTO-70 review conflict | automation/bundles/active/US-AUTO-73/ | Semantic companion filtering was partially implemented, but the story failed to converge on committed-HEAD rerun. Repeated `blocked_review_artifact_fidelity` indicates producer/consumer drift caused by duplicated non-runtime companion-filter logic across pipeline stages. Return condition: resume only after US-AUTO-74 centralizes non-runtime companion artifact classification into a single reusable source of truth used by producer and downstream review consumers. |
| US-AUTO-74 | Centralize semantic companion-filter contract | Introduce one shared source of truth for semantic companion filtering so producer and downstream review/analyze/classify/gate consumers reuse the same non-runtime companion artifact classification and stop drifting on rerun | follow-up | Planned | P1 | Draft bundle | US-AUTO-73 contract-level unblocker | N/A | Contract-level follow-up required to unblock US-AUTO-73. Scope: centralize non-runtime companion artifact classification, migrate producer and downstream consumers to the shared contract, remove duplicated filtering logic, and add cross-consumer contract coverage. Non-goals: manual-finish, rerun policy, operator UX, review override logic, replay optimization. |
