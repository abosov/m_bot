...
| US-AUTO-70 | Rerun-preflight stable-review recomputation for companion-filtered stories | Recompute the effective filtered review surface before rerun/gate so companion-filtered stories do not fail acceptance because rerun-preflight still evaluates the unadjusted surface | follow-up | Bundle Drafted | P1 | Materialize and validate bundle | US-AUTO-69 split follow-up | automation/bundle_packs/US-AUTO-70.bundle.md | Scope must stay limited to `automation/scripts/run_story.sh` and `tests/test_run_story.py`; do not modify `automation/run_codex_task.sh`; must fail closed if the effective filtered review surface cannot be derived deterministically; completion of this story is the return condition for resuming the parked US-AUTO-69 line |
