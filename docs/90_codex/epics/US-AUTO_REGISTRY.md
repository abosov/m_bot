...
| US-AUTO-51 | Manual-finish review continuation contract | Align analyze / classify / gate so a confirmed non-converging rerun that was manually finished and committed can continue through pinned downstream review on committed HEAD without weakening generic stale-run fail-closed behavior | follow-up | In Progress | P1 | Merge US-AUTO-51, then resume parked US-AUTO-28-F1 branch | US-AUTO-28-F1 / US-AUTO-47 | automation/bundle_packs/US-AUTO-51.bundle.md | Active blocker-follow-up for the parked `US-AUTO-28-F1` implementation. Analyze/manual-finish continuation must stay non-contradictory even when older pinned gate evidence exists from pre-manual-finish `review_head_mismatch` rejects. Keep `US-AUTO-28-F1` blocked until this story is merged; scope remains downstream continuation contract only. |
