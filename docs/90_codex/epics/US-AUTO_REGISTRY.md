...
- A new contract-level follow-up need was confirmed during US-AUTO-73: semantic companion filtering currently relies on duplicated non-runtime companion classification across producer and downstream review consumers; centralization into one shared source of truth remains tracked as US-AUTO-74 to reduce future drift risk and simplify the contract surface.
- A new contract-level gap was confirmed while attempting US-AUTO-74: centralizing semantic companion classification alone does not converge cleanly under committed-HEAD rerun because review-fidelity recomputation still lacks an explicitly separated consumption/projection contract for the centralized helper output.
...
- Attempted implementation of US-AUTO-74 confirmed that producer and downstream helper centralization is not by itself a sufficient convergence boundary: repeated committed-HEAD reruns continued to end in `blocked_review_artifact_fidelity`, indicating that centralized classification and review-fidelity consumption/projection must be treated as distinct contract concerns.
...
1. US-AUTO-75 — clarify review-fidelity projection contract for centralized semantic companion filtering
...
| US-AUTO-74 | Centralize semantic companion-filter contract | Introduce one shared source of truth for semantic companion filtering so producer and downstream review/analyze/classify/gate consumers reuse the same non-runtime companion artifact classification and reduce future drift risk | follow-up | Blocked | P1 | Resume only after US-AUTO-75 lands | Follow-up from US-AUTO-73 | automation/bundles/active/US-AUTO-74/ | Bundle was materialized and implementation was attempted on parked branch `feat/us-auto-74-centralize-companion-filter-contract`, but repeated committed-HEAD reruns still ended in `blocked_review_artifact_fidelity`. Centralized helper migration partially converged, yet review-fidelity recomputation did not reach a stable fixed point. Return condition: resume only after US-AUTO-75 clarifies how review-fidelity consumers must project centralized semantic companion-filter outputs. |
| US-AUTO-75 | Clarify review-fidelity projection contract for centralized semantic companion filtering | Define how review-fidelity recomputation must consume/project centralized semantic companion-filter outputs so committed-HEAD reruns converge without reopening producer/consumer helper centralization loops | follow-up | Planned | P1 | Draft bundle | US-AUTO-74 contract-level unblocker | N/A | Direct unblocker for blocked US-AUTO-74. Scope: specify and implement the distinct review-fidelity consumption/projection contract for the centralized helper output, preserve fail-closed committed-HEAD review semantics, and prevent back-and-forth fixes between shared classification and downstream review consumers. Non-goals: manual-finish, rerun policy, operator UX, replay optimization, or redoing US-AUTO-74 helper centralization. |
