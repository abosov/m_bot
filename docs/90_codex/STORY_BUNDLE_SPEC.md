...
- For automation workflow stories that add repository-visible lifecycle evidence, document the canonical ledger artifact path explicitly so downstream stories read one stable source instead of reconstructing history from scattered run artifacts.
