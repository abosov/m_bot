...
- For automation workflow stories that rely on pinned review outputs, the gate must consume the selected run's existing `ai_review_result.md` and `review_classification.md` as read-only source-of-truth artifacts and must not silently rerun upstream review/classification steps.
...
- For automation workflow stories that mark a ledger path as ephemeral, the same exact path contract (no directory-level or wildcard exclusions) must be reused for dirty-tree checks, diff generation, and exit cleanup in run/review/finalize scripts so real implementation drift remains strictly visible.
- For automation workflow stories that start from a clean primary checkout, make the top-level runner the single owner of rollback: baseline capture must happen before repository-visible writes, rollback must stay armed until the explicit success boundary, and failed or interrupted runs must restore tracked state plus remove run-owned untracked artifacts instead of leaving partial automation output behind.
- For automation workflow stories that add automatic rollback, treat rollback failure as a hard visible failure state; never downgrade it into a nominally successful run.
