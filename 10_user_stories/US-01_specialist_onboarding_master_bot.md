...
## Статус сценария
- **Implemented (MVP):** старт онбординга, ввод публичного имени, подключение personal bot, Google OAuth callback, выбор существующего календаря с пагинацией, интеграция календаря, финализация с переходом в personal bot.
...
5. Календарный шаг:
   - выбрать существующий календарь (реализовано):
     ...
     - после выбора запускается интеграция (create+delete test event);
     - при неуспешной интеграции сохраняется статус failed и предлагается повторная проверка через `calendar:smoke`;
     - доступна смена календаря через экран выбора действия (выбрать существующий календарь).
   - если нужен отдельный календарь — специалист создаёт его вручную в Google Calendar.
6. Интеграция календаря (create+delete test event).
7. После успешной интеграции система автоматически применяет дефолты (идемпотентно, только если значения отсутствуют):
...
- выбран рабочий календарь;
- `last_smoke_test_status = ok` (интеграция календаря успешна).

Важно: master bot не проводит опрос по этим настройкам, а только авто-применяет стартовую конфигурацию после интеграции.
...
2. Подключить Google.
3. Выбрать существующий календарь и дождаться успешной интеграции.
4. Убедиться, что пояснение про создание отдельного календаря вручную в Google Calendar присутствует в UI.
5. Проверить, что `specialist.status=active` и personal bot отвечает на `/start`.
