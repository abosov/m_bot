...
- Если Google не прислал refresh_token:
  - при наличии старого refresh_token в БД — подключение считается успешным,
  - без сохранённого токена — специалист получает инструкцию по переподключению (consent + offline).
...
## Чек-лист в /start
Статус в master bot показывается по отдельным пунктам:
- Имя,
- Личный бот,
- Google OAuth,
- Календарь выбран (отдельный пункт),
- Smoke-test.

Онбординг считается полностью завершённым только когда выполнены все пункты, включая выбор календаря.
