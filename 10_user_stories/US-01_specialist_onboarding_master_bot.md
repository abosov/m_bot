...
8. После перехода в personal bot запускается wizard базовых настроек (продолжение онбординга, уже в US-02):
   - рабочие дни недели;
   - интервалы по дню (дефолт: `09:00–12:00`, `13:00–17:00`, `17:00–21:00`);
   - длительность сессии (дефолт 60) и буфер (дефолт 10, только для расчёта слотов);
   - `max_sessions_per_day` (дефолт 4) и `slot_step_min` (`{60,30,15,10}`, дефолт 15).
