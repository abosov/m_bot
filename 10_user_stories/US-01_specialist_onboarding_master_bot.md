...
9. При первом входе в `owner_panel` personal bot дополнительно проверяет, что дефолты действительно заполнены в профиле и weekly availability; если чего-то нет, значения авто-доприменяются и сразу показываются кнопки для изменения (расписание / длительность+буфер / лимиты+шаг / TZ).
