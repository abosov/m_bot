...
### Как работать на VPS
- На VPS **не используется** `.env.local`.
- Все переменные окружения задаются через окружение сервиса (systemd/Docker/панель хостинга).
