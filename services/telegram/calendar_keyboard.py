...
def format_calendar_button_text(item: dict, *, is_current: bool = False) -> str:
    ...
    calendar_type = "Primary" if bool(item.get("primary")) else "Secondary"
    current_marker = "\n✅ Current" if is_current else ""
    return f"{summary}\n{timezone}\n{calendar_type}{current_marker}"
        ...
        rows.append(
            [
                types.InlineKeyboardButton(
                    text=f"📅 {format_calendar_button_text(item, is_current=bool(current_calendar_id and item_id == current_calendar_id))}",
