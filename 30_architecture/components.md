...
- mutate-операции cache (set/update/cleanup) защищены `asyncio.Lock`;
...
- применение ограничений:
  ...
  - UI layer behaviour: UI (когда предлагает выбрать «утро/день/вечер») показывает интервалы availability в исходном виде (как их задал специалист); интервалы со значениями `start=NULL` и `end=NULL` считаются отключёнными и отображаются как `—`
  - Domain validation behaviour: Склейка стык-в-стык интервалов выполняется только для domain-валидации слота и генерации доступных стартов; UI (когда предлагает выбрать «утро/день/вечер») показывает интервалы в исходном виде (как их задал специалист) (реализация через `merge_intervals()` / `is_time_range_allowed()` / `generate_day_slot_starts()`)
