...
- применение ограничений:
  ...
  - UI layer behaviour: интервалы availability отдаются/показываются строго в исходном виде, заданном специалистом (без склейки); интервалы со значениями `start=NULL` и `end=NULL` считаются отключёнными и клиенту не показываются
  - Domain validation behaviour: склейка стык-в-стык интервалов availability в единый диапазон только внутри проверки допустимости слота (через `merge_intervals()`)
