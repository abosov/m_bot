...
- `local` — локальная разработка (readyz по умолчанию отключён)
...
- На VPS `/readyz` включён и используется для мониторинга готовности.
...
- GitHub Actions — временный мониторинг до полноценного uptime на VPS.
