...
MVP-решение для uptime:
- GitHub Actions по расписанию (каждые 5 минут) делает `GET /readyz`.
- Если ответ не 200 — job падает, что даёт минимальный сигнал о доступности.
- Это временная мера; позже заменяем на полноценный uptime service.
