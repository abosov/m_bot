...
Мониторинг uptime на VPS:
- `/readyz` включён и используется для проверки готовности.
- Uptime-сервис (например, UptimeRobot/Healthchecks) делает `GET /readyz`
  и уведомляет при статусе не `200`.
- GitHub Actions больше **не** используется как uptime-мониторинг
  (оставляем его для CI — проверок кода).
