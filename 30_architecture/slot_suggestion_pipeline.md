...
Pipeline покрывает путь от запроса клиента «показать доступные слоты» до ответа UI с 3–4 рекомендациями.
...
6. **RankingService (логический слой внутри AvailabilityService или отдельный модуль)**
   - расчет score,
   - сортировка,
   - ограничение выдачи до 3–4 рекомендаций.
...
**Правило:** запись возможна только на следующий календарный день и только если `specialist_now <= 21:00` предыдущего дня.  
**Результат:**
...
7. **Ограничение до 3–4 слотов** — Pipeline 11.
...
**Ход pipeline (кратко):**
- Pipeline 2 cutoff проходит (`now <= 21:00` накануне),
