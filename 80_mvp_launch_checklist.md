...
- [ ] Пояснение про создание отдельного календаря вручную в Google Calendar добавлено в UI
...
- [ ] specialist может задать минимальный буфер между сессиями
- [ ] В owner wizard есть дефолты: Пн–Пт, 3 интервала (`09:00–12:00`, `13:00–17:00`, `17:00–21:00`), длительность 60, буфер 10
- [ ] В owner wizard задаются `max_sessions_per_day` (дефолт 4) и `slot_step_min` (только `{60,30,15,10}`, дефолт 15)
...
### Слоты
- [ ] Слоты считаются по weekly availability (Склейка стык-в-стык интервалов выполняется только для domain-валидации слота и генерации доступных стартов; UI (когда предлагает выбрать «утро/день/вечер») показывает интервалы в исходном виде (как их задал специалист). Не требуется завершать сессию в конце первого интервала при domain-проверке)
- [ ] Бронирование/изменение доступны только если до начала слота осталось не менее cancel_window_hours (по умолчанию 12 часов) по времени specialist (единое ограничение: операции доступны только если до начала слота >= cancel_window_hours.
- [ ] Учитывается `max_sessions_per_day` (дефолт 4, не больше 4 в день)
- [ ] Учитывается `slot_step_min` (только `{60,30,15,10}`, дефолт 15)
