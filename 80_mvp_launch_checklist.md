...
### Слоты
- [ ] Слоты считаются по weekly availability (склейка стык-в-стык интервалов выполняется только для domain-валидации слота; UI показывает интервалы в исходном виде; не требуется завершать сессию в конце первого интервала при domain-проверке)
