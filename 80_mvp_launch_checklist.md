...
- [ ] В owner wizard есть дефолты: Пн–Пт, 3 интервала (утро/день/вечер), длительность 60, буфер 10
- [ ] В owner wizard задаются `max_sessions_per_day` и `slot_step_min`
...
### Слоты
- [ ] Слоты считаются по weekly availability (стык-в-стык интервалы объединяются)
- [ ] Бронирование/изменение доступны только на следующий календарный день и только до 21:00 предыдущего дня по времени specialist (next day + cutoff 21:00)
