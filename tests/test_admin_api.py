...
@pytest.mark.asyncio
async def test_admin_ui_reset_test_data_flow_preflight_confirm_execute(tmp_path, monkeypatch):
    app, database = load_app(tmp_path, monkeypatch, admin_key="secret", admin_ui_password="ui-secret")
    ...
    async with database.async_session_factory() as session:
        ...
        session.add(
            database.SpecialistProfile(
                specialist_id=specialist_id,
                public_name="Reset Specialist",
                owner_tg_user_id=7001,
                owner_tg_username="reset_owner",
                specialist_timezone="UTC",
            )
        )
        session.add(
            database.SpecialistPublicProfile(
                id=profile_id,
                specialist_id=specialist_id,
                public_slug="reset-spec",
                display_name="Reset Specialist",
                specialization="Psychology",
                client_bot_username="reset_bot",
                is_published=False,
                created_at=now,
                updated_at=now,
            )
        ...
        session.add(
            database.SpecialistCalendarSettings(
                specialist_id=specialist_id,
                calendar_id="calendar-reset",
                calendar_summary="Reset",
        ...
        session.add(
            database.Appointment(
                ...
                end_at_utc=now + timedelta(minutes=30),
                booking_state=database.BookingState.pending,
                idempotency_key="reset-flow-appt",
            )
        )
        session.add(
            database.AppointmentReminder(
                appointment_id=appointment_id,
                specialist_id=specialist_id,
                reminder_type=database.ReminderType.h2,
                due_at_utc=now,
                created_at_utc=now,
            )
        )
        ...
        session.add(
            database.OutboxEvent(
                ...
                payload_json={"specialist_id": str(specialist_id), "appointment_id": str(appointment_id)},
            )
        )
        session.add(database.NotificationLog(outbox_event_id=outbox_id, target="telegram"))
        session.add(
            database.SpecialistPublicMedia(
                profile_id=profile_id,
                media_type="image",
                file_key="media/reset-1.jpg",
                sort_order=1,
                created_at=now,
            )
        )
    ...
    assert preflight["delete_counts"] == {
        "clients": 1,
        "appointments": 1,
        "notifications": 1,
        "media": 1,
    ...
    async with database.async_session_factory() as session:
        ...
        profile_exists = await session.scalar(
            select(database.SpecialistProfile.specialist_id).where(database.SpecialistProfile.specialist_id == specialist_id)
        )
        ...
        calendar_exists = await session.scalar(
            select(database.SpecialistCalendarSettings.specialist_id).where(database.SpecialistCalendarSettings.specialist_id == specialist_id)
        )
        bot_exists = await session.scalar(
            select(database.TelegramBot.specialist_id).where(database.TelegramBot.specialist_id == specialist_id)
        )
        ...
        notifications_count = await session.scalar(select(func.count()).select_from(database.NotificationLog))
        media_count = await session.scalar(select(func.count()).select_from(database.SpecialistPublicMedia))
        audit_actions = (
            await session.execute(
                select(database.AdminAuditLog.action)
                .where(database.AdminAuditLog.target_id == specialist_id)
                .order_by(database.AdminAuditLog.created_at.asc())
            )
        ).scalars().all()

    assert specialist_exists == specialist_id
    assert profile_exists == specialist_id
    assert oauth_exists == specialist_id
    assert calendar_exists == specialist_id
    assert bot_exists == specialist_id
    assert appointments_count == 0
    assert clients_count == 0
    assert notifications_count == 0
    assert media_count == 0
    assert "reset_test_data_preflight_requested" in audit_actions
    assert "reset_test_data_execute_requested" in audit_actions
    assert "reset_test_data_committed" in audit_actions


@pytest.mark.asyncio
async def test_admin_ui_reset_test_data_rejects_execute_without_confirmation(tmp_path, monkeypatch):
    app, database = load_app(tmp_path, monkeypatch, admin_key="secret", admin_ui_password="ui-secret")
    ...
    token = preflight_response.json()["confirmation_token"]
    ...
    execute_response = client.post(
        ...
        json={
            ...
            "confirmation_phrase": f"RESET TEST DATA {specialist_id}",
        },
        cookies={"admin_session": session_cookie, "admin_csrf": csrf_cookie},
        headers={"X-CSRF-Token": csrf_cookie},
    )
    ...
    assert execute_response.json() == {"detail": "PRECONDITION_FAILED"}


@pytest.mark.asyncio
async def test_admin_ui_reset_test_data_forbidden_for_non_test(tmp_path, monkeypatch):
    app, database = load_app(tmp_path, monkeypatch, admin_key="secret", admin_ui_password="ui-secret")
...
async def _execute_us_ad_13_reset_flow_for_assertions(tmp_path, monkeypatch):
    app, database = load_app(tmp_path, monkeypatch, admin_key="secret", admin_ui_password="ui-secret")
    ...
    async with database.async_session_factory() as session:
        ...
        session.add(
            database.SpecialistProfile(
                specialist_id=specialist_id,
                public_name="US-AD-13 QA",
                owner_tg_user_id=8401,
                owner_tg_username="qa_owner",
                specialist_timezone="UTC",
            )
        )
        session.add(
        ...
        session.add(
            database.SpecialistCalendarSettings(
                specialist_id=specialist_id,
                calendar_id="qa-calendar",
                calendar_summary="QA Calendar",
                calendar_time_zone="UTC",
                source=database.SpecialistCalendarSource.selected,
            )
        )
        ...
        session.add(
            database.Appointment(
                ...
                client_id=client_row.client_id,
                start_at_utc=now,
                end_at_utc=now + timedelta(minutes=45),
                booking_state=database.BookingState.pending,
                idempotency_key="qa-reset-appt",
            )
        )
        await session.commit()
...
async def test_us_ad_13_reset_deletes_runtime_data(tmp_path, monkeypatch):
    outcome = await _execute_us_ad_13_reset_flow_for_assertions(tmp_path, monkeypatch)
...
@pytest.mark.asyncio
async def test_us_ad_13_reset_preserves_identity(tmp_path, monkeypatch):
    outcome = await _execute_us_ad_13_reset_flow_for_assertions(tmp_path, monkeypatch)
...
@pytest.mark.asyncio
async def test_us_ad_13_reset_preserves_oauth(tmp_path, monkeypatch):
    outcome = await _execute_us_ad_13_reset_flow_for_assertions(tmp_path, monkeypatch)
...
@pytest.mark.asyncio
async def test_us_ad_13_reset_preserves_calendar(tmp_path, monkeypatch):
    outcome = await _execute_us_ad_13_reset_flow_for_assertions(tmp_path, monkeypatch)
