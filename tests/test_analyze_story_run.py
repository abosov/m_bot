...
def test_analyze_story_run_accepts_valid_refresh_evidence(tmp_path: Path) -> None:
    ...
    run_dir = make_run_dir(root_dir, "US-AUTO-60", "2026-05-02_12-00-00_refresh")
    (run_dir / "manifest.md").write_text(
        "# Refresh Manifest\n\n"
        "- story_id: US-AUTO-60\n"
        "- branch: feature/us-auto-60\n"
        ...
        json.dumps(
            {
                "story_id": "US-AUTO-60",
                "current_head": head,
                "current_branch": "feature/us-auto-60",
                "base_ref": "main",
                "merge_base": head,
                "refresh_mode": "no_codex_review_evidence_refresh",
                "codex_invoked": False,
    ...
    result = run_script(root_dir, "US-AUTO-60", run_dir=run_dir)
    assert result.returncode == 0, result.stderr
...
def test_analyze_story_run_blocks_invalid_refresh_metadata(tmp_path: Path) -> None:
    ...
    run_dir = make_run_dir(root_dir, "US-AUTO-60", "2026-05-02_12-10-00_refresh")
    (run_dir / "manifest.md").write_text(
        "# Refresh Manifest\n\n"
        "- story_id: US-AUTO-60\n"
        "- branch: feature/us-auto-60\n"
...
def test_analyze_story_run_surfaces_invalid_ai_review_artifact(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, result.stderr
    assert "AI review: present (invalid: ai_review_normalization_failed)" in result.stdout
    ...
    assert "RUN STATUS: CHECK AI REVIEW OUTPUT (invalid artifact: ai_review_normalization_failed)" in result.stdout
    assert "RUN STATUS: READY TO CLASSIFY" not in result.stdout
...
def test_analyze_story_run_surfaces_prompt_echo_ai_review_artifact(tmp_path: Path) -> None:
    root_dir = tmp_path / "repo"
    run_dir = make_run_dir(root_dir, "US-AUTO-19", "2026-03-16_13-07-00")
    ...
    prompt_text = (
        ...
        "# AI Review Result\n\n"
        "PASS\n"
        "This is a long prompt line to trigger echo detection during analyze validation.\n"
        "This is a long prompt line to trigger echo detection during analyze validation.\n"
        "This is a long prompt line to trigger echo detection during analyze validation.\n"
        "This is a long prompt line to trigger echo detection during analyze validation.\n"
    )
...
def test_analyze_story_run_reports_non_converging_rerun_manual_finish_boundary(tmp_path: Path) -> None:
    root_dir = tmp_path / "repo"
    root_dir.mkdir(parents=True, exist_ok=True)
    ...
    (root_dir / "services" / "story_loop.py").write_text("second\n", encoding="utf-8")
    (root_dir / "tests" / "test_story_loop.py").write_text("assert True\n", encoding="utf-8")
    subprocess.run(
        ["git", "add", "services/story_loop.py", "tests/test_story_loop.py"],
        cwd=root_dir,
        check=True,
    )
    subprocess.run(["git", "commit", "-m", "second head"], cwd=root_dir, check=True, capture_output=True, text=True)
    second_head = current_head(root_dir)
    ...
    assert result.returncode == 0, result.stderr
    assert "Current stage: blocked_non_converging_rerun" in result.stdout
    assert "Latest valid stage: run_artifacts_ready" in result.stdout
    assert "Resume safety: blocked" in result.stdout
    assert "Next recommended command: none" in result.stdout
...
def test_analyze_story_run_keeps_non_safety_classification_loop_cap_as_escalation(tmp_path: Path) -> None:
    root_dir = tmp_path / "repo"
    root_dir.mkdir(parents=True, exist_ok=True)
    ...
    story_id = "US-AUTO-58"
    run_ids = [
        "2026-05-02_12-31-00_refresh",
        "2026-05-02_12-41-00_refresh",
        "2026-05-02_12-51-00_refresh",
    ]
    ...
    for run_id in run_ids:
        ...
        (run_dir / "review_classification.md").write_text(
          ...
          "**Findings By Classification**\n\n"
          "`MERGE BLOCKER`\n"
          "1. No safety issue, but this is non-safety polish/scope churn.\n\n"
          "MERGE RECOMMENDATION: reject\n",
          encoding="utf-8",
        )
...
def test_analyze_story_run_allows_normal_run_artifacts_when_changed_files_do_not_repeat(tmp_path: Path) -> None:
    ...
    (run_dir / "changed_files.txt").write_text("services/story_b.py\n", encoding="utf-8")
    (run_dir / "pytest.txt").write_text("4 passed\n", encoding="utf-8")
    (run_dir / "review_bundle.md").write_text("# Review Bundle\n", encoding="utf-8")
    (run_dir / "chatgpt_review_prompt.md").write_text("# Prompt\n", encoding="utf-8")
    (run_dir / "diff.patch").write_text("diff --git a/x b/x\n", encoding="utf-8")
...
def test_analyze_story_run_ignores_companion_only_non_converging_signal_for_code_only_story(
    tmp_path: Path,
) -> None:
    root_dir = tmp_path / "repo"
    root_dir.mkdir(parents=True, exist_ok=True)
    ...
    (root_dir / "services").mkdir(parents=True, exist_ok=True)
    (root_dir / "services" / "story_loop.py").write_text("second\n", encoding="utf-8")
    subprocess.run(["git", "add", "services/story_loop.py"], cwd=root_dir, check=True)
    subprocess.run(["git", "commit", "-m", "second head"], cwd=root_dir, check=True, capture_output=True, text=True)
    second_head = current_head(root_dir)
    ...
    previous_run = make_run_dir(root_dir, "US-AUTO-70", "2026-03-27_10-00-00")
    (previous_run / "manifest.md").write_text(
        "# Codex Run Manifest\n\n"
        f"- starting_head: {first_head}\n"
        "- codex_exit_code: 0\n"
    ...
    run_dir = make_run_dir(root_dir, "US-AUTO-70", "2026-03-27_11-00-00")
    (run_dir / "manifest.md").write_text(
        "# Codex Run Manifest\n\n"
        f"- starting_head: {second_head}\n"
        "- codex_exit_code: 0\n"
...
def test_analyze_story_run_recomputes_filtered_non_converging_surface_for_code_only_story(
    tmp_path: Path,
) -> None:
    root_dir = tmp_path / "repo"
    root_dir.mkdir(parents=True, exist_ok=True)
    ...
    (root_dir / "services").mkdir(parents=True, exist_ok=True)
    (root_dir / "services" / "story_a.py").write_text("a\n", encoding="utf-8")
    subprocess.run(["git", "add", "services/story_a.py"], cwd=root_dir, check=True)
    subprocess.run(["git", "commit", "-m", "first head"], cwd=root_dir, check=True, capture_output=True, text=True)
    first_head = current_head(root_dir)

    (root_dir / "services" / "story_b.py").write_text("b\n", encoding="utf-8")
    subprocess.run(["git", "add", "services/story_b.py"], cwd=root_dir, check=True)
    subprocess.run(["git", "commit", "-m", "second head"], cwd=root_dir, check=True, capture_output=True, text=True)
    second_head = current_head(root_dir)
...
def test_analyze_story_run_uses_recomputed_filtered_diff_surface_for_non_converging_detection(
    tmp_path: Path,
) -> None:
    root_dir = tmp_path / "repo"
    root_dir.mkdir(parents=True, exist_ok=True)
    ...
    review_artifact_base = current_head(root_dir)
    (root_dir / "services").mkdir(parents=True, exist_ok=True)
    (root_dir / "services" / "story_loop.py").write_text("return 'first'\n", encoding="utf-8")
    subprocess.run(["git", "add", "services/story_loop.py"], cwd=root_dir, check=True)
    subprocess.run(["git", "commit", "-m", "first head"], cwd=root_dir, check=True, capture_output=True, text=True)
    first_head = current_head(root_dir)

    (root_dir / "services" / "story_loop.py").write_text("return 'second'\n", encoding="utf-8")
    subprocess.run(["git", "add", "services/story_loop.py"], cwd=root_dir, check=True)
    subprocess.run(["git", "commit", "-m", "second head"], cwd=root_dir, check=True, capture_output=True, text=True)
    second_head = current_head(root_dir)
    ...
    (run_dir / "diff.patch").write_text(
        subprocess.run(
            ["git", "diff", review_artifact_base, second_head, "--", "services/story_loop.py"],
            cwd=root_dir,
            check=True,
            capture_output=True,
            text=True,
...
def test_analyze_story_run_allows_exact_manual_finish_continuation_for_non_converging_rerun(
    tmp_path: Path,
) -> None:
    root_dir = tmp_path / "repo"
    root_dir.mkdir(parents=True, exist_ok=True)
    ...
    subprocess.run(["git", "commit", "-m", "manual finish"], cwd=root_dir, check=True, capture_output=True, text=True)
...
def test_analyze_story_run_blocks_manual_finish_review_until_final_head_compliance_is_proven(
    tmp_path: Path,
) -> None:
    root_dir = tmp_path / "repo"
    root_dir.mkdir(parents=True, exist_ok=True)
...
def test_analyze_story_run_marks_gate_approved_manual_finish_continuation_ready_for_merge_when_final_head_compliance_is_proven(
    tmp_path: Path,
) -> None:
    root_dir = tmp_path / "repo"
    root_dir.mkdir(parents=True, exist_ok=True)
    ...
    previous_run = make_run_dir(root_dir, "US-AUTO-55", "2026-03-31_07-13-21")
    (previous_run / "manifest.md").write_text(
        "# Codex Run Manifest\n\n"
        f"- starting_head: {first_head}\n"
        "- codex_exit_code: 0\n"
    ...
    run_dir = make_run_dir(root_dir, "US-AUTO-55", "2026-03-31_07-29-45")
    (run_dir / "manifest.md").write_text(
        "# Codex Run Manifest\n\n"
        f"- starting_head: {second_head}\n"
        "- codex_exit_code: 0\n"
    ...
    result = run_script(root_dir, "US-AUTO-55", run_dir=run_dir)
...
def test_analyze_rejects_invalid_semantic_projection(tmp_path: Path) -> None:
    root_dir = tmp_path / "repo"
    root_dir.mkdir(parents=True, exist_ok=True)
    ...
    story_id = "US-AUTO-75"
    run_dir = make_run_dir(root_dir, story_id, "analyze-invalid-projection")
