...
def test_analyze_story_run_keeps_manual_finish_ready_when_gate_artifact_has_stale_head_mismatch(
    tmp_path: Path,
) -> None:
    root_dir = tmp_path / "repo"
    root_dir.mkdir(parents=True, exist_ok=True)
    ...
    assert result.returncode == 0, result.stderr
    assert "Evidence HEAD Consistency: continuation-ready" in result.stdout
    assert "Current stage: manual_finish_ready_for_review" in result.stdout
    assert "Current stage: blocked_review_gate_rejected" not in result.stdout
    assert "Next recommended command: AUTOMATION_RUN_DIR=" in result.stdout
    assert "automation/scripts/review_gate_story_run.sh" in result.stdout
    assert "US-AUTO-47" in result.stdout
    assert "RUN STATUS: READY (manual finish committed; review can continue on current HEAD)" in result.stdout
