...
### 4.1 Lead time
Запись доступна не ранее чем через 2 часа:
- `slot_start >= now + 2h`

Lead time рассчитывается в UTC (через перевод времени), чтобы избежать ошибок
на границах часовых поясов.

### 4.2 Период “текущая неделя до конца”
Период вычисляется в timezone клиента:
- `period_start_client = now_client + 2h`
