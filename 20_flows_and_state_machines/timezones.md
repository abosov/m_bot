...
### 4.1 Cutoff-правило записи (next day + 21:00)
Запись и изменение доступны только на следующий календарный день и только
до 21:00 предыдущего дня в timezone specialist.

Правило рассчитывается в TZ specialist:
- если `now_specialist <= 21:00`, доступна запись только на следующий день;
- если `now_specialist > 21:00`, следующий день недоступен и запись возможна только на послезавтра.
...
- склейке стык-в-стык интервалов availability в единый диапазон только на уровне domain-валидации доступности слота (в UI интервалы остаются в исходном виде),
