...
После нажатия «Сменить часовой пояс» клиент видит список из 3 частей (страниц).
Внизу страниц 1 и 2 показывается кнопка «еще» для перехода 1 -> 2 -> 3.
При изменении:
- в интерфейсе кнопки отображаются в формате `UTC±X — Город`, но в базе сохраняется IANA timezone name
  (например, кнопка `UTC+1 — Берлин` сохраняет `Europe/Berlin`)
- ручной ввод IANA timezone остаётся доступен (например `Europe/Berlin`)
